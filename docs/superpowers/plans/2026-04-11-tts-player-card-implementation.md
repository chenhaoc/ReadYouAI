...
**Goal:** Add a fixed player card with weighted seek, stable queue ordering, and per-article playback bookmarks.

**Architecture:** Move progress state from a single current-item model to a per-article bookmark model in the queue controller. Render one fixed control card above the queue list and reduce list items back to selection plus queue management.
...
- Modify: `app/src/test/java/me/ash/reader/ui/page/home/reading/queue/TtsQueueSheetStateTest.kt`
